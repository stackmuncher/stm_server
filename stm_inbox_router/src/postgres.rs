...
impl Dev {
    ...
    pub(crate) async fn queue_up_for_update(
        pg_client: &Client,
        owner_id: &String,
        gh_login_gist_latest: &Option<String>,
    ) -> Result<(), Error> {
        ...
        let rows = match pg_client
            .execute(
                "select stm_queue_up_dev_report($1::varchar, $2::varchar)",
                &[owner_id, gh_login_gist_latest],
            )
            .await
        {
            Ok(v) => v,
