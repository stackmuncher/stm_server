...
async fn main() -> Result<(), Error> {
    ...
    let config_owned = Config::new().await;
...
mod proxy {
    ...
    /// A debug version of the runner. Invokes the handler and returns the output to the queue.
    pub(crate) async fn run(config: &Config) -> Result<(), Error> {
        loop {
            ...
            info!("New msg");
            // invoke the handler
            crate::handler::my_handler(payload.event, payload.ctx, config).await?;

            delete_message(receipt_handle).await?;
            info!("Msg deleted");
        }
    }
