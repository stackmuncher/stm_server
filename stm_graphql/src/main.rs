...
async fn main() -> Result<(), Error> {
    ...
    #[cfg(not(debug_assertions))]
    return lambda_runtime::run(service_fn(handler::my_handler)).await;
}
...
mod proxy {
    ...
    const REQUEST_QUEUE_URL_ENV: &str = "STM_GRAPHQL_PROXY_REQ"; // add your queue URL there
    const RESPONSE_QUEUE_URL_ENV: &str = "STM_GRAPHQL_PROXY_RESP"; // add your queue URL there
    ...
    pub(crate) async fn run() -> Result<(), Error> {
        ...
        loop {
            ...
            // invoke the handler
            let response = crate::handler::my_handler(LambdaEvent::new(payload.event, payload.ctx)).await?;
