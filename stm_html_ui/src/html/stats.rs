...
pub(crate) struct Stats {
    ...
    stm_stats_deletion_queue_counts: Value,
}
...
pub(crate) async fn html(config: &Config, html_data: HtmlData) -> Result<HtmlData, ()> {
    ...
    let stm_stats_deletion_queue_counts = elastic::get_stm_stats(&config.es_url, "stm_stats_deletion_queue_counts", 60);
    ...
    let jobs = vec![
        ...
        stm_stats_deletion_queue_counts,
    ];
    ...
    let stats_jobs = Stats {
        ...
        stm_stats_contributor_counts: response
            ...
            .expect("stm_stats_contributor_counts ES query failed"),
        stm_stats_deletion_queue_counts: response
            .pop()
            .expect("stm_stats_deletion_queue_counts ES query failed")
            .expect("stm_stats_deletion_queue_counts ES query failed"),
    };
